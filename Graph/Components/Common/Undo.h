...
public:
  ...
  void Undo();
  void Redo();
  void Pop(); //Pop the top item without undo; Used to cancel a call to Push()
  bool CanUndo() const;
  bool CanRedo() const;
  void Clear();
  void ClearRedo();
  ...
  template<class TFunctor> const TFunctor* Peek()
  {
    if(UndoStack.empty())
      return NULL;
    if(TUndoItem<TFunctor> *Item = dynamic_cast<TUndoItem<TFunctor>*>(UndoStack.back().get()))
      return &Item->Functor;
    return NULL;
  }
};
