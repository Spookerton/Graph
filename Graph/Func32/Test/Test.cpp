...
#ifdef _WIN64
void std::_Debug_message(const char *Str, const char *File, unsigned int Line)
{
	std::clog << Str << " at " << File << ": " << Line << std::endl;
	BOOST_ASSERT(0);
}
#else
void std::_Debug_message(const char *Str, const char *Str2)
{
...
void Test()
{
...
#ifndef _WIN64 //long double is the same as double on Win64
  ...
  //acos(1.5) returns 0.9624236501 when compiled for Win32 (boost 1.39) and
  //-0.9624236501 when compiled for Win64 (boost 1.50). Both seems to be equally good.
#ifdef _WIN64
...
//Prevents runtime access violation when compiled with C++ Builder XE6 for Win64
//I don't know what is causing this, but my best guess is a linker bug.
//An alternative fix is to add the Func32 files directly instead of linking with Func32.a.
#if defined(__BORLANDC__) && defined(_WIN64)
