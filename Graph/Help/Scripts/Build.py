...
ToolsDir = "..\\..\\Tools\\"
# Language (File name), lang ID as found in Tools\xsl\common, replaced text
Languages = [
    ...
    ("Chinese (Simplified)", "zh_cn",   {"$LabelWidth": "4em"}),
]
...
def HandleLanguage(Language, Lang, Dict):
    ...
    HtmlPath = "..\\html\\%s\\" % Language.replace(" ", "_").lower()
    StyleSheet = "..\\xsl\\htmlchunk.xsl" if not os.path.exists("..\\xsl\\%s\\htmlchunk.xsl" % (Language,)) else "..\\xsl\\%s\\htmlchunk.xsl" % (Language,)
    os.system('XCopy ..\\Source\\*.css %s /Q /Y /I > NUL' % HtmlPath)
    os.system('xsltproc.exe --nonet --xinclude --noout --output %s "%s" %s > NUL'  % (HtmlPath, StyleSheet, InFile))
    os.system('XCopy ..\\Images\\Common\\*.png %s\\images /I /Q /Y > NUL' % (HtmlPath,))
    os.system('XCopy "..\\Images\\%s\\*.*" %s\\images /I /Q /Y > NUL' % (Language,HtmlPath))
