...
static PyObject* VclModule_Dir(PyObject *self, PyObject *arg)
{
	...
	PyObject *List = PyList_New(TypeList->Count + GlobalCount + GlobalFunctionCount + 1);
	int Index;
  ...
  PyList_SetItem(List, Index++, ToPyObject("FormatSettings"));
	return List;
}
//---------------------------------------------------------------------------
...
PyObject* InitPyVcl()
{
  ...
  //See http://bugs.python.org/issue18426
	...
	PyObject_SetAttrString(PyVclModule, "FormatSettings", VclRecord_Create(&FormatSettings, Context.GetType(FormatSettingsTypeInfo), NULL)); //This will leak a reference, which is okay
  ...
  Py_INCREF(VclObject_Type);
	PyModule_AddObject(PyVclModule, "VclObject", reinterpret_cast<PyObject*>(VclObject_Type)); //This steals a reference
