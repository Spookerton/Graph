...
public:
  TDraw(TCanvas *Canvas, TData *pData, const std::string &ThreadName, bool UseGdiPlus);
  ~TDraw();
  void DrawAll();
  ...
  void SetSizeMul(double ASizeMul) {SizeMul = ASizeMul;}
  ...
  void DrawLegend(TContext &Context, const TRect &LegendRect);
};
} //namespace Graph
#endif
