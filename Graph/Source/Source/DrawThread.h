...
private:
  ...
  void EquationLoop(TRelation &Relation, std::vector<BYTE> &Polylines, bool Loop);
  static bool CheckResult1(const long double Result[3]);
  static bool CheckResult2(const long double Result[3]);
  void AddPoint(const TPoint &P, std::vector<BYTE> &EquationBitmap);
  TPoint IndexToPoint(unsigned I);
  unsigned PointToIndex(const TPoint &P);
  void BitmapToPolylines(std::vector<BYTE> &EquationBitmap, std::vector<TPoint> &PolylinePoints, std::vector<int> &PolylineCounts);
