...
namespace Python
{
  enum TPluginEvent
  {
    ...
    peShowForm,
    peCloseForm,
    peFunctionListUpdated,
    peShortCut
  };

  extern PyObject *PyEFuncError;
  extern PyObject *PyEGraphError;
	typedef boost::variant<int, double, String, std::wstring, PyObject*, TObject*> TPyVariant;
  ...
  inline PyObject* ToPyObject(const TPoint &P) {return CreateTuple(P.x, P.y);}
  inline PyObject* ToPyObject(const Func32::TCoordSet<> &P) {return CreateTuple(P.t, P.x, P.y);}

  template<typename T>
  PyObject* ToPyObject(const std::vector<T> &V)
  {
    PyObject *Tuple = PyTuple_New(V.size());
    for(unsigned I = 0; I < V.size(); I++)
      PyTuple_SET_ITEM(Tuple, I, ToPyObject(V[I]));
    return Tuple;
  }
